<template>
  <Transition name="modal">
    <div class="project-details__outer fixed inset-4 flex justify-center items-center z-[9998] pointer-events-none"
      :key="props.project.title" :style="transitionStyle" v-if="props.show">
      <div ref="modalRef" :class="{ 'mobile-fullscreen': !isDesktop }"
        ...
        <div class="project-details__inner flex flex-col items-center">
          <div
            ...
            <h2 class="text-lg font-semibold text-neutral tracking-wider">{{ props.project.title }}</h2>
...
const asyncComponent = computed(() => props.project.id ? defineAsyncComponent(() => import(`@/components/projects/${props.project.id}.vue`)) : null);
...
const isDesktop = ref(viewportWidth.value >= 640 && !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent));
...
const dragStyle = computed(() => {
  if (!isDesktop.value || !isDragging.value && modalPosition.value.x === 0 && modalPosition.value.y === 0) {
    return {}
  } else {
    return {
      'position': 'absolute',
...
const updateDeviceType = () => {
  ...
  isDesktop.value = viewportWidth.value >= 640 && !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
...
.project-details {
  ...
  width: 100%;
  height: fit-content;
  background-color: oklch(var(--base-100)/.9);
...
.modal-enter-active,
.modal-leave-active {
  transition: transform .5s cubic-bezier(.63, -0.34, .31, 1.27);
}
